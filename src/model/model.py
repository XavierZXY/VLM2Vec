...
from src.model.baseline_backbone.lamra.lamra_qwen25_inference import (
    LamRAQwen25VL,
)
from src.model.baseline_backbone.phi3_v.modeling_phi3_v import Phi3VForCausalLM
from src.model.baseline_backbone.llava_next import (
    ...
    def encode_input(self, input):
        if getattr(self, "model_backbone", None) == INTERNVIDEO2:
            if "input_ids" in input.keys():
                ...
                return vfeat
        elif getattr(self, "model_backbone", None) in [
            GME,
            LamRA,
            LamRA_QWEN2_5,
        ]:
            ...
            for imgs in input["images"]:
                ...
                if isinstance(imgs, list):
                    ...
                    assert not isinstance(
                        imgs, list
                    )  # make sure we have extracted the middle frame and it is no longer a list
                    images.append(imgs)
                else:
                    images.append(imgs)
    ...
    def load(cls, model_args: ModelArguments, is_trainable=True, **kwargs):
        ...
        if model_args.model_backbone in {
            ...
            base_model = GmeQwen2VL(
                model_args.model_name, processor=kwargs["processor"]
            )
