    ...
    def get_rotary_embedding(
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            inv_freq = 1.0 / (
                self.rope_theta
                ** (
                    torch.arange(0, dim, 2, device=device, dtype=torch.float)
                    / dim
                )
            )
...
class LLaDASequentialBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            q, k, v = self.att_proj(self.attn_norm(x)).split(
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def reset_parameters(self):
        ...
        # NOTE: the standard deviation for these weights does not depend on the layer.
        init_weights(
            self.config, self.q_proj, d=self.config.d_model, layer_id=None
        )
        init_weights(
            self.config, self.k_proj, d=self.config.d_model, layer_id=None
        )
        init_weights(
            self.config, self.v_proj, d=self.config.d_model, layer_id=None
        )
        init_weights(
            self.config, self.ff_proj, d=self.config.d_model, layer_id=None
        )
        init_weights(
...
class LLaDAModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        self._activation_checkpoint_fn: Callable = (
            activation_checkpoint_function(self.config)
        )
    ...
    def forward(
        ...
        if (
            ...
            if attention_mask is not None:
                ...
                # it can produce NaNs.
                ensure_finite_(
                    attention_bias, check_neg_inf=True, check_pos_inf=False
                )
        ...
        if prefix_length is not None:
            ...
            prefix_length = (
                prefix_length.reshape(-1).to(torch.int32).to(x.device)
            )
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                layer_past = (
                    None
                    if past_key_values is None
                    else past_key_values[block_idx]
                )
                if (
                    (
                        self.activation_checkpointing_strategy
            ...
            raise NotImplementedError(
                "Block group size > 1 is not yet supported in LLaDA"
            )
            for group_idx, block_group in enumerate(
                ...
                layers_past = (
                    ...
                    if past_key_values is None
                        ...
                        group_idx * self.config.block_group_size : (
                            group_idx + 1
                        )
...
class LLaDAModelLM(PreTrainedModel):
        ...
        if labels is not None:
            ...
            if ENFORCE_NUM_ITEMIN_BATCH:
                assert num_items_in_batch is not None, (
                    "num_items_in_batch should be set in the environment variable"
                )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        model_inputs["use_cache"] = kwargs.pop(
            "use_cache", self.config.use_cache
        )
        return model_inputs
