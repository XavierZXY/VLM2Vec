...
from llava.utils import (
    rank0_print,
    process_video_with_pyav,
    process_video_with_decord,
)
from src.model.vlm_backbone.llava.model.language_model.modeling_lavida import (
...
def preprocess_gemma(
    ...
    conv: conversation_lib.Conversation = (
        conversation_lib.default_conversation.copy()
    )
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if has_image:
                ...
                instruction_len = (
                    len(tokenizer(parts[0]).input_ids) - 1
                )  # Ignore <bos>
...
def preprocess_llama3(
    ...
    unmask_tokens_idx = [
        tokenizer.convert_tokens_to_ids(tok) for tok in unmask_tokens
    ]
    ...
    for i, source in enumerate(sources):
        ...
        convs = [get_conv_owner(x) for x in source]
        gpt_indices = [
            i
            for i, (source, content, key) in enumerate(convs)
            if source == "human"
        ]  # human,gpt
        if SELECT_ONE_INDEX:
            selected_indices = rng.choice(gpt_indices)
        else:
            selected_indices = None
        if selected_indices is not None:
            assert selected_indices + 1 < len(source)
            source = source[selected_indices : selected_indices + 2]
        ...
        for idx, conv in enumerate(source):
            ...
            try:
                ...
                role = conv["from"]
                content = conv["value"]
            if idx == 0 and selected_indices is not None:
                if "<image>\n" not in content:
                    content = "<image>\n" + content
            role = roles.get(role, role)
...
def preprocess_llada(
    ...
    for i, source in enumerate(sources):
        ...
        system_conv = tokenizer.apply_chat_template(
            [{"role": "system", "content": system_message}], tokenize=False
        )
        if system_conv.endswith(
            "<|start_header_id|>assistant<|end_header_id|>\n\n"
        ):
            _len = len("<|start_header_id|>assistant<|end_header_id|>\n\n")
            system_conv = system_conv[:-_len]
        input_id += tokenizer(system_conv)["input_ids"]
        target += [IGNORE_INDEX] * len(input_id)
        # n_source = len(source)
        # assert n_source % 2 == 0
        convs = [get_conv_owner(x) for x in source]
        ...
        for idx, conv in enumerate(source):
            ...
            try:
                ...
                content = conv["value"]
            if idx == 0:
                if "<image>" not in content and has_image:
                    content = "<image>\n" + content
            role = roles.get(role, role)
            ...
            # hack in case sth is broken
            if encode_id.startswith("<|startoftext|>"):
                encode_id = encode_id[len("<|startoftext|>") :]
            if encode_id.endswith(
                "<|start_header_id|>assistant<|end_header_id|>\n\n"
            ):
                _len = len("<|start_header_id|>assistant<|end_header_id|>\n\n")
                encode_id = encode_id[:-_len]
            non_maskable_tokens = 0
            if encode_id.startswith(
                "<|start_header_id|>assistant<|end_header_id|>\n\n"
            ):
                # hack, this is 6 tokens
                non_maskable_tokens = len([26346, 598, 10450, 126347, 198, 198])
            encode_id = tokenizer(encode_id)["input_ids"]
            # very manual hack
            input_id += encode_id
            # if role in ["user", "system"]:
            if idx == 0:
                ...
                target += [IGNORE_INDEX] * non_maskable_tokens
                target += encode_id[non_maskable_tokens:]
...
def preprocess_dream(
    ...
    # unmask_tokens = ["<|im_start|>", "<|im_end|>", "<|end_header_id|>", "<|eot_id|>", "\n\n"]
    ...
    for i, source in enumerate(sources):
        ...
        for idx, conv in enumerate(source):
            ...
            # hack in case sth is broken
            # breakpoint()
            if encode_id.startswith("<|beginoftext|>"):
                encode_id = encode_id[len("<|beginoftext|>") :]
            # breakpoint()
            if encode_id.startswith(
                ...
                encode_id = encode_id[_len:]
            non_maskable_tokens = 0
            if encode_id.startswith("<|im_start|>assistant\n"):
                # hack, this is 6 tokens
                non_maskable_tokens = len([151644, 77091, 198])
            encode_id = tokenizer(encode_id)["input_ids"]
            # very manual hack
            input_id += encode_id
            # if role in ["user", "system"]:
...
def preprocess_plain(
    ...
    for target, source in zip(targets, sources):
        tokenized_len = len(
            tokenizer_image_token(source[0]["value"], tokenizer)
        )
...
def preprocess(
    ...
    """
    #    assert conversation_lib.default_conversation.version == "llada" # hack
    # breakpoint()
    if (
    ...
    if conversation_lib.default_conversation.version == "llada":
        return preprocess_llada(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "dream":
        return preprocess_dream(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
    ...
    def get_tokenize_len(prompts):
        return [
            len(tokenizer_image_token(prompt, tokenizer)) for prompt in prompts
        ]
...
class LazySupervisedDataset(Dataset):
    def __init__(
        ...
        if "{" in data_path and "}" in data_path:
            ...
            self.prepend_folder = (
                False  # we load image root from yaml and ignore args
            )
            ...
            with open(data_path, "r") as file:
                cur_data_dict = json.load(file)
                rank0_print(
                    f"Loaded {len(cur_data_dict)} samples from {data_path}"
                )
                self.list_data_dict.extend(cur_data_dict)

        rank0_print(
            f"Loaded {len(self.list_data_dict)} samples from {data_path}"
        )
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if image_aspect_ratio == "highres":
            ...
            image_aspect_ratio == "anyres" or "anyres_max" in image_aspect_ratio
        ):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                print(
                    f"[Try #{attempt_idx}] Failed to fetch sample {i}. Exception:",
                    e,
                )
                time.sleep(1)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        labels = [
            _labels[: self.tokenizer.model_max_length] for _labels in labels
        ]
        ...
        if "image" in instances[0]:
            ...
            batch["modalities"] = [
                im[2] for im_list in images for im in im_list
            ]
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if (
        ...
        if training_args.model_max_length is None:
            ...
            overwrite_config["max_sequence_length"] = (
                training_args.model_max_length
            )
    ...
    if (
        ...
        overwrite_config["mm_spatial_pool_stride"] = (
            model_args.mm_spatial_pool_stride
        )
        overwrite_config["mm_spatial_pool_out_channels"] = (
            model_args.mm_spatial_pool_out_channels
        )
        overwrite_config["mm_spatial_pool_mode"] = (
            model_args.mm_spatial_pool_mode
        )
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            if training_args.dev == "test3":
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(
                    ...
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(
                        torch.bfloat16 if training_args.bf16 else None
                    ),
                    low_cpu_mem_usage=False,
                    **customized_kwargs,
                )
            ...
            if training_args.load_vlm:
                vision_kwargs = dict(
                    ...
                    mm_patch_merge_type=getattr(
                        model_args, "mm_patch_merge_type", "spatial_unpad"
                    ),
                )
            if training_args.dev == "test3":
                # config = AutoConfig.from_pretrained(model_args.model_name_or_path)
                if "config" not in customized_kwargs:
                    customized_kwargs["config"] = AutoConfig.from_pretrained(
                        model_args.model_name_or_path
                    )
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.get_model().initialize_vision_modules(
            model_args=model_args, fsdp=training_args.fsdp, config=model.config
        )
        ...
        if data_args.image_grid_pinpoints is not None:
            if (
                ...
                data_args.image_grid_pinpoints = [
                    [dim * patch_size for dim in pair]
    ...
    if training_args.lmms_eval_generate_tasks:
