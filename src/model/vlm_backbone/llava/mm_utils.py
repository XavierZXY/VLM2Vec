...
def auto_pad_images(image, grid_params):
    ...
    candidate_resolutions = [
        (w / h, w, h) for w in grid_params for h in grid_params
    ]
...
def process_anyres_image(image, processor, grid_pinpoints):
    ...
    if DEBUG_FIX_PADDING:
        image_padded_square = expand2square(
            image, tuple(int(x * 255) for x in processor.image_mean)
        )
        image_original_resize = image_padded_square.resize(
            (processor.size["shortest_edge"], processor.size["shortest_edge"])
        )
    else:
...
def process_images(images, image_processor, model_cfg):
    ...
    if image_aspect_ratio == "highres":
        ...
        for image in images:
            image = process_highres_image_crop_split(
                image, model_cfg, image_processor
            )
            new_images.append(image)
